...
def run_benchmark(world_size, rank, gpu_id, tokenizer, args):
    ...
    sorted_input_ids = all_input_ids
    sorted_padded_gen_lens = padded_gen_lens
    ...
    if rank==0:
        ...
        print(f'Forward: {total_forward}, Time: {stop-start}, FPS: {np.mean(fpss)}, TPS: {np.mean(tpss)}, TPF: {np.mean(tpfs)}')
